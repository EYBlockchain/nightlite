 ...
 * @returns {Number} senderCommitmentTreeIndex - the index of the token within the Merkle Tree.  This is required for later transfers/joins so that Alice knows which 'chunks' of the Merkle Tree she needs to 'get' from the FTokenShield contract in order to calculate a path.
 */
async function burn(
    tokenId,
    secretKey,
    ...
    const { fTokenShieldJson, fTokenShieldAddress, account } = blockchainOptions;
    const fTokenShield = ethersUtils.getContractWithSigner(
      fTokenShieldJson,
      fTokenShieldAddress,
      account,
    );
  ...
  const path = await computePath(account, fTokenShield, commitment, commitmentIndex).then(
    result => {
      return {
  ...
  await fTokenShield.burn(proof, finalInputs, vkId);
