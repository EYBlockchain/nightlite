...
import "./Poseidon.sol"; // import contract with MiMC function

contract MerkleTree is Poseidon {
    ...
    function insertLeaf(bytes32 leafValue) public returns (bytes32 root) {
        ...
        for (uint level = 0; level < treeHeight; level++) {
            ...
            if (nodeIndex % 2 == 0) {
                ...
                output[0] = hashBytes32(input); // mimc hash of concatenation of each node
